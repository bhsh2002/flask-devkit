...
توفر DevKit نظامًا آليًا قويًا لتدقيق تغييرات قاعدة البيانات وتسجيل أحداث النظام الهامة.

### تدقيق قاعدة البيانات

بمجرد استخدام `BaseRepository` لنماذجك، يتم تسجيل جميع عمليات `CREATE` و `UPDATE` و `DELETE` تلقائيًا في جدول `devkit_audit_log`. يوفر هذا سجلاً كاملاً وغير قابل للتغيير لتغييرات البيانات، بما في ذلك:

- المستخدم الذي قام بالإجراء (إذا تمت مصادقته).
- اسم الجدول والمفتاح الأساسي للسجل الذي تم تغييره.
- فرق JSON بين `old_values` و `new_values` لعمليات `UPDATE`.

لا تتطلب هذه الميزة أي تكوين وتعمل مباشرة.

### تسجيل أحداث النظام

تقوم DevKit تلقائيًا بتسجيل أحداث الأمان والتطبيقات الرئيسية في مسجل Flask القياسي، مما يوفر سياقًا حاسمًا لتصحيح الأخطاء والمراقبة:

- **عمليات تسجيل الدخول الناجحة والفاشلة**: يتم تسجيل استدعاءات `login_user` مع اسم المستخدم والنتيجة.
- **الاستثناءات غير المعالجة**: يقوم معالج أخطاء مركزي بتسجيل تتبع أي استثناء غير معالج قبل إرجاع استجابة `500 Internal Server Error`.
- **فشل التفويض**: يتم تسجيل JWTs المفقودة أو غير الصالحة.
...
### السيناريو 8: الحذف الناعم، الأرشفة والاستعادة

للتطبيقات التي تتطلب أن تكون البيانات قابلة للاسترداد، توفر DevKit آلية قوية ومدمجة للحذف الناعم والأرشفة.
...
يمكنك التحكم في العناصر التي يتم إرجاعها في نقطة نهاية القائمة باستخدام معامل الاستعلام `deleted_state`.

-   **العناصر النشطة (الافتراضي)**: `/api/v1/my-models` أو `/api/v1/my-models?deleted_state=active`
-   **جميع العناصر (النشطة + المحذوفة)**: `/api/v1/my-models?deleted_state=all`
-   **العناصر المحذوفة فقط**: استخدم نقطة النهاية المخصصة الجديدة: `/api/v1/my-models/deleted`

يمكن تمكين نقطة النهاية الجديدة هذه عبر تكوين التوجيه:
...
register_crud_routes(
    ...
    service=my_service,
    schemas=my_schemas,
    routes_config=crud_config, # اسم المعلمة المصحح
)
```
سيؤدي هذا إلى إنشاء نقطة نهاية `POST /my-auditable-models/<id>/restore`.

#### 5. الحذف الدائم والأرشفة

إذا كنت بحاجة إلى حذف سجل بشكل دائم، فاستخدم دالة `force_delete`. بدلاً من محوها، سيتم تحويل بيانات السجل إلى JSON ونقلها إلى جدول `archived_records` مركزي لأغراض التدقيق.
